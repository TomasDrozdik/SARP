  ...
  for (int run = 0; run < 1; ++run) {
    for (double threshold = 3; threshold < 4; threshold += 0.1) {
      ...
      auto [sp, network, event_generators] =
          ...
          //  LinearStaticOctreeAddresses(RoutingType::SARP, 10,
          //  sarp_parameters); SquareStaticOctreeAddresses(RoutingType::SARP,
          //  10, 10, sarp_parameters);
          CubeStaticOctreeAddresses(RoutingType::SARP, 3, 3, 3,
                                    sarp_parameters);
