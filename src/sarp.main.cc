...
int main() {
  ...
  for (int run = 0; run < 1; ++run) {
    for (double treshold = 3; treshold <= 4.5; treshold += 0.05) {
      for (double update_treshold = 0; update_treshold <= 0.5; update_treshold += 0.05) {
        Parameters::Sarp sarp_parameters = {
            .neighbor_cost = Cost(1, 0.1),
            .compact_treshold = treshold,
            .update_treshold = update_treshold,
            .ratio_variance_treshold = 0.9,  // arbitrary
