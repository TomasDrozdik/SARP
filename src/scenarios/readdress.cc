...
BootThreeReaddressNew(Parameters::Sarp sarp_parameters) {
  ...
  node_generation.boot_time = std::make_unique<FiniteTimeGenerator>(
      std::vector<Time>({0, 0, 0, 200000}));
  node_generation.initial_addresses = nullptr;
  node_generation.initial_positions = std::make_unique<FinitePositionGenerator>(
      std::vector<Position>({Position(0, 0, 0), Position(100, 0, 0),
                             Position(200, 0, 0), Position(100, 100, 0)}));
  ...
  custom_events.push_back(std::make_unique<ReaddressEvent>(
      301000, TimeType::ABSOLUTE, *network, only_empty));
...
StaticCubeReaddress(Parameters::Sarp sarp_parameters) {
  ...
  event_generators.push_back(std::make_unique<ReaddressEventGenerator>(
...
AddNewToGrid(unsigned x, unsigned y, unsigned add_count,
  ...
  bool only_empty = true;
  custom_events.push_back(std::make_unique<ReaddressEvent>(
      400000, TimeType::ABSOLUTE, *network, only_empty));

  // Add specific traffic to every other node.
  for (NodeID new_node_id = node_count; new_node_id < node_count + add_count;
       ++new_node_id) {
...
AddNewToCube(unsigned x, unsigned y, unsigned z, unsigned add_count,
  ...
  event_generators.push_back(std::make_unique<NodeGenerator>(
      *network, add_count, RoutingType::SARP,
      std::make_unique<RandomTimeGenerator>(range<Time>{300000, 301000}),
      std::make_unique<RandomPositionGenerator>(range<Position>{
          Position(0, 0, 0), Position(x * 100, y * 100, z * 100)}),
      nullptr));
