...
class SimulationParameters {
  ...
  friend std::ostream &operator<<(
      std::ostream &os, const SimulationParameters &simulation_parameters);
 ...
 public:
  ...
  Time get_duration() const { return duration_; }

  uint32_t get_ttl_limit() const { return ttl_limit_; }

  uint32_t get_connection_range() const { return connection_range_; }
...
class Statistics {
 ...
 public:
  ...
  void RegisterUndeliveredPacket() { ++undelivered_packets_; }
  ...
  void RegisterBrokenConnectionsSend() { ++broken_connection_sends_; }

  void RegisterDetectedCycle() { ++cycles_detected_; }

  void RegisterTTLExpire() { ++ttl_expired_; }

  void set_network(const Network *network) { this->network_ = network; }

  size_t get_delivered_packets_count() const { return delivered_packets_; }

  size_t get_undelivered_packets_count() const { return undelivered_packets_; }
  ...
  size_t get_routing_overhead_size() const { return routing_overhead_size_; }

  size_t get_cycles_detected_count() const { return cycles_detected_; }
